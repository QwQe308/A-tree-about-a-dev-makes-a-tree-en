...
// data = a function returning the content (actually HTML)
	...
	Vue.component('resource-display', {
		...
		<div style="margin-top: -13px">
			...
			<br><br>
			<span v-if="tmp[layer].showBest">Best:{{tmp[layer].resource}} {{formatWhole(player[layer].best)}}<br></span>
			<span v-if="tmp[layer].showTotal">Total:{{formatWhole(player[layer].total)}} {{tmp[layer].resource}}<br></span>
		</div>
		`
	})
